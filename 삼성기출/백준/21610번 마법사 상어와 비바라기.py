...
# ***********주의*********** 좌표가 0부터 시작인지, 1부터 시작인지
# 1번 행과 N번 행 연결, 1번 열과 N번 열 연결
# 비바라기 시전하면  (N, 1), (N, 2), (N-1, 1), (N-1, 2) -> (N-1, 0), (N-1, 1), (N-2, 0), (N-2, 1)에 비구름생김
# 구름에 이동을 M번 명령한다. i번째 이동 명령은 방향di와 거리si로 이루어져 있다.
# 방향은 8개로 좌, 좌상, 상, 상우, 우, 우하, 하 , 하좌
# 이동 명령 시 순서
#1. 모든 구름이 di방향으로 si칸 이동
#2. 각 구름에서 비가 내려 구름 있는 칸의 바구니에 저장된 물의 양이 1 증가
#3. 구름이 모두 사라짐
#4. 2에서 물이 증가한 칸 (r, c)에 대각선 방향으로 거리가 1인 칸에 물이 있는 바구니의 수 만큼 (r, c)에 있는 바구니의 물의 양 증가
#   이때는 격자판의 경계를 넘어가면 안됨
#5. 바구니에 저장된 물의 양이 2 이상인 모든 칸에 구름이 생기고, 물의 양이 2 줄어든다.
# 이때 구름이 생기는 칸은 3에서 구름이 사라진 칸이 아니어야 한다.
# M번 이동이 모두 끝난 후 바구니에 들어 있는 물의 양의 합을 구해보자.

n,m=map(int, input().split()) #nxn 격자판, 구름 이동 명령 m번
board=[list(map(int, input().split())) for _ in range(n)] #nxn 격자판
move_data=[list(map(int, input().split())) for _ in range(m)]

# 8방향
dx=[0, -1, -1, -1, 0, 1, 1, 1]
dy=[-1, -1, 0, 1, 1, 1, 0, -1]
cloud_pos=[[n-1, 0], [n-1, 1], [n-2, 0], [n-2, 1]] #현재 비구름의 위치 ***주의*** -1을 해줘야한다. 격자판의 인덱스가 달라서

for i in range(m): #구름의 이동을 m번 명령
    cloud_visited = [[0] * n for _ in range(n)]  # 구름이 방문한 여부
    for j in range(len(cloud_pos)): #구름 위치를 모두 돌면서 di, si만큼 이동시켜줌
        d, s=move_data[i] # 구름의 i번째 이동 방향, 이동거리
        x, y =cloud_pos[j] # 구름의 현재 위치
        nx=(x+dx[d-1]*s)%n # 1(0)번행과 n(n-1)번행이 연결되어 있으므로
        ny=(y+dy[d-1]*s)%n
        #2. 각 구름에서 비가 내려 물의양 1 증가
        board[nx][ny]+=1
        # 구름이 생성되었다가 사라진 칸임을 체크
        cloud_visited[nx][ny]=1


    # 2에서 물이 증가한칸(r,c)에 대각선 방향으로 거리가 1인 칸이 몇개인지 구해서 그만큼 (r, c)에 더해줌
    ddx=[-1, -1, 1, 1] # 대각선 방향
    ddy=[-1, 1, 1, -1]
    for i in range(n):
        for j in range(n):
            cnt = 0  # 대각선 4방향에 대해 물이 있는지 여부
            if cloud_visited[i][j]==1: # 물이 증가한 칸에 대해서만
                for k in range(4): # 네 방향에 대해서
                    cx=i+ddx[k]
                    cy=j+ddy[k]
